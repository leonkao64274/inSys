 ...
 * 
 * Description:
 *      InvCore EMV invSys 系統 - ACS 後台管理系統 "內容加密金鑰設定" 控制器類別
 *
 * Modify History:
 ...
 * @author   LeonKao
 */
@SessionAttributes("cekCriteriaForm")
@Controller
public class CekController {
	 ...
	 */
	@ModelAttribute("cekCriteriaForm")
	public CekCriteriaForm initialModel() {
		return new CekCriteriaForm();
	}
	 ...
	 */
	@RequestMapping(value = "/cek/query")
	@AcessRightCheck(accessId = "cek")
	@DoOperationLog(accessId = "cek", operation = "Q", targetObject = "T_KMS_INFO")
	public String query(@Valid @ModelAttribute("cekCriteriaForm") CekCriteriaForm form, BindingResult result,
			Model model) {
		// 操作記錄 - 查詢條件
		try {
			LogContextHolder.putLogAttribute(LogContextHolder.ATTR_DATA_QUERY, JsonUtil.writeValueAsBytes(form));
		} catch (JsonProcessingException e) {
			logger.warn("cek query json query error");
		}
		// 1. 準備 "模型的組態" 信息
		...
		// 3. 依據查詢條件進行查詢
		Page<KmsInfo> queryResult = kmsInfoService.findByCriterial(KmsInfoService.KEY_TYPE_CEK,
	 ...
	 */
	@AcessRightCheck(accessId = "cek")
	@RequestMapping(value = "/cek/add", method = RequestMethod.GET)
	 ...
	 */
	@AcessRightCheck(accessId = "cek")
	@RequestMapping(value = "/cek/add", method = RequestMethod.POST)
	@DoOperationLog(accessId = "cek", operation = "A", targetObject = "T_KMS_INFO")
	public String add(@Valid @ModelAttribute("form") KmsInfo form, BindingResult result, Model model) {
		...
		try {
			...
			logger.warn("cek add json after error");
		}
		// 1. 準備 "模型的組態" 信息
		this.prepareConfigModel(model);
		...
		KmsInfo kek = kmsInfoService.findActiveKek();
		if (kek == null) {
			// 設定錯誤訊息，返回原編輯作業畫面顯示
			model.addAttribute(WebKeyConst.ERRORS,
					messageSource.getMessage("warn.kms-info.kek-not-set", null, LocaleContextHolder.getLocale()));
			return "/cek/add";
		}

		// 4. 保存 "ISSUER KEK" 的數據實體。
		try {
			form.setKekKeyAlias(kek.getKeyAlias());
			form.setKeyType(KmsInfoService.KEY_TYPE_CEK);
			form.setStatus(KmsInfoService.STATUS_DISABLED);
			kmsInfoService.save(form);
		} catch (ResourceAlreadyExistException ex) {
			logger.warn("Failed to add CEK !", ex);
			// 設定錯誤訊息，返回原編輯作業畫面顯示
			model.addAttribute(WebKeyConst.ERRORS,
	 ...
	 */
	@AcessRightCheck(accessId = "cek")
	@RequestMapping(value = "/cek/enable", method = RequestMethod.GET)
	@DoOperationLog(accessId = "cek", operation = "E", targetObject = "T_KMS_INFO")
	public String enable(@RequestParam Long oid, Model model) {
		...
		KmsInfo kms = kmsInfoService.findById(oid) ;
		...
		try {
			...
			// 啟用金鑰
			kms = kmsInfoService.enableKey(oid);
			// 操作記錄 - 異動前/後資料
			LogContextHolder.putLogAttribute(LogContextHolder.ATTR_DATA_AFTER,JsonUtil.writeValueAsBytes(kms));
			...
			logger.warn("KmsInfo write after log error");
		}
		// 操作記錄 - 操作結果
		LogContextHolder.putLogAttribute(LogContextHolder.ATTR_RESULT, true);
		// 5. 執行成功：返回查詢條件頁面並重新查詢資料
	...
	private String recallQuery(Model model) {
		...
		CekCriteriaForm form = (CekCriteriaForm) model.asMap().get("cekCriteriaForm");
