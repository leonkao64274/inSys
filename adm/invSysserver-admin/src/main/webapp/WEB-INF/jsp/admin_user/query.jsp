...
<div id="sitemesh-header">
	<spring:message code="ui.admin-user.management" />
	<!-- 群組使用者管理 -->
</div>
...
<div id="sitemesh-content">
	<form:form id="form1" action="#" method="POST"
		modelAttribute="adminUserCriteriaForm">
		<!-- 錯誤訊息區 -->
						...
						<div class="row">
								...
								<spring:bind path="criteriaAdminGroupOid">
									<div class="form-group ${status.error ? 'has-error' : ''}">
										...
										<form:select path="criteriaAdminGroupOid" class="form-control">
											...
											<form:options
												items="${adminUserCriteriaForm.configAdminGroupList}"
							...
							<div class="col-md-3">
								<spring:bind path="criteriaAccount">
									<div class="form-group ${status.error ? 'has-error' : ''}">
										<label class="control-label"> <spring:message
												code="ui.admin-user.account" /> <!-- 使用者代號 -->
										</label>
										<form:input path="criteriaAccount" class="form-control"
											maxlength="20" />
			...
			<div class="col-md-12">
				<div class="pull-right">
					<a href="#" id="btnQuery" class="btn btn-primary"><spring:message
							code="btn.query" /> <!--查詢--></a> <a href="./add" id="btnAdd"
						...
						<thead>
							<tr class="bg-primary">
								<th class="text-nowrap"><spring:message
										code="ui.admin-user.account" /> <!-- 使用者代號 --></th>
								<th class="text-nowrap"><spring:message
										code="ui.admin-user.user-name" /> <!-- 使用者名稱 --></th>
								<th class="text-nowrap"><spring:message
										code="ui.admin-user.group" /> <!-- 群組 --></th>
								<th class="text-nowrap"><spring:message
										code="ui.admin-user.department" /> <!-- 部門 --></th>
								<th class="text-nowrap"><spring:message
										code="ui.admin-user.tel" /> <!-- 電話 --></th>
								<th class="text-nowrap"><spring:message
										code="ui.admin-user.ext" /> <!-- 分機 --></th>
								<th class="text-nowrap"><spring:message
										code="ui.admin-user.email" /> <!-- 電子郵箱 --></th>
						...
						<tbody>
							<c:forEach items="${queryResult.content}" var="entity">
								<tr>
									...
									<td>${entity.email}</td>
									<td><c:if
											test="${entity.userType eq '1'}">
											<a href="./edit?oid=<c:out value="${entity.oid}"/>"
												class="btn btn-sm btn-default"> <spring:message
													code="btn.update" /> <!--修改-->
											</a>
											
												<a href="./delete?oid=<c:out value="${entity.oid}"/>"
													id="btnDelete_${entity.oid}" class="btn btn-sm btn-default">
													<spring:message code="btn.delete" /> <!--刪除-->
												</a>
