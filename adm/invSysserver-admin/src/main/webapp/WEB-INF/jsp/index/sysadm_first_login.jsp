...
// 		});
		...
		$("#btnSubmit").click(function() {
			if (validate() == true) {
				$form1.attr("action", "./sysadm_first_login");
		...
		function validate() {
			...
			if (password.length == 0) {
				...
				$encryptNewPassword.val(window.sha1($newAccount.val() + $newPassword.val())); 
				...
				<div class="login-panel panel panel-default">
					<div class="panel-heading">
						<h3 class="panel-title">
							<spring:message text="" code="ui.change-password.sysadm-first-login" /><!-- 首次登入系統請重新設定管理員帳號密碼 -->
						</h3>
					</div>
					<div class="panel-body">
						<form:form id="form1" action="#" modelAttribute="form">
							<fieldset>
								...
								<!-- (2) 用戶名稱 -->
								<spring:bind path="newAccount">
								<div class="form-group ${status.error ? 'has-error' : ''}">
									<label class="control-label">
										<spring:message text="" code="ui.change-password.new-account" />${req}<!-- 登錄帳號 -->
									</label>
									<form:input path="newAccount" cssClass="form-control" maxlength="100"
											id="newAccount" />
