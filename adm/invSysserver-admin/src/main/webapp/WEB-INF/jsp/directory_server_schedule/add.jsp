...
<div id="sitemesh-header">
	<h3 class="page-header">
		<spring:message code="ui.card-scheme-download-setting" />
		<!--卡段下載設定-->
	</h3>
</div>
						...
						<div class="row">
							...
							<div class="col-md-6">
								<spring:bind path="serverUrl">
									<div class="form-group ${status.error ? 'has-error' : ''}">
										<label class="control-label"> <spring:message
												code="ui.serverUrl" />${req}
											<!--服務器URL	-->
										</label>
										<form:input path="serverUrl" cssClass="form-control" />
										<form:errors path="serverUrl" />
										<!-- 錯誤綁定 -->
									</div>
								</spring:bind>
							</div>
						...
						<div class="row">
								...
								<spring:bind path="opRunningType">
									<div class="form-group ${status.error ? 'has-error' : ''}">
										...
										<form:select class="form-control" path="opRunningType">
											...
											<option value="ALL"
												<c:if test="${'ALL'==form.opRunningType}">selected</c:if>><spring:message
													code="ui.opRunningType.ALL" /><!--全部同步	--></option>
											<option value="DIF"
												<c:if test="${'DIF'==form.opRunningType}">selected</c:if>><spring:message
													code="ui.opRunningType.DIF" /><!--差異同步	--></option>
										</form:select>
										<form:errors path="opRunningType" />
										<!-- 錯誤綁定 -->
									</div>
								</spring:bind>
							</div>
							...
							<div class="col-md-6">
								<spring:bind path="opDuration">
									<div class="form-group ${status.error ? 'has-error' : ''}">
										<label class="control-label"> <spring:message
												code="ui.opDuration" />${req}
											<!--執行週期	-->
										</label>
										<form:input path="opDuration" cssClass="form-control"
											maxlength="2" data-type="numeric" />
										<form:errors path="opDuration" />
										<!-- 錯誤綁定 -->
									</div>
								</spring:bind>
							</div>
								...
								<div class="form-group">
									<label class="control-label"></label> <a href="#"
										id="btnSubmit" class="btn btn-primary"><spring:message
											code="btn.save" /> <!--確認--></a> <a href="#" id="btnReset"
										class="btn btn-default"><spring:message code="btn.reset" />
										<!--重設--></a> <a href="#" id="btnCancel" class="btn btn-default"><spring:message
