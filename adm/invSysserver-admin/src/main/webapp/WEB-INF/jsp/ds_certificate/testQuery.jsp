...
<!-- 												請選擇 -->
<%-- 											</form:option> --%>
<%-- 											<form:options items="${issuerConfigModel}" itemValue="oid" --%>
<%-- 												itemLabel="issuerName" /> --%>
<!-- 											發卡銀行訊息 -->
<%-- 										</form:select> --%>
<!-- 									</div> -->
<%-- 								</spring:bind> --%>
...
<%-- 										code="ui.issuer.issuer-name" /> <!-- 銀行 --></th> --%>
								...
								<th class="text-nowrap"><spring:message
										code="ui.issuer-cert.Signature-key-base-code-recognition" /> <!-- 密鑰基碼識別 --></th>
								<th class="text-nowrap"><spring:message
...
<%-- 									<td><c:out value="${issuerCert.issuer.issuerName}" /></td> --%>
									...
									<td>
									<fmt:parseDate var="createTime" pattern="yyyyMMddHHmmss" value="${issuerCert.notBefore}" />
